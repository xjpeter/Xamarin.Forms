...
#pragma warning restore 1998
		...
		void OnElementScrollToRequested(object sender, ScrollToRequestedEventArgs e)
		{
			...
			ScrollTo(scrollArgs.GetValueGroup(), scrollArgs.GetValueItem(), e.Position, e.ShouldAnimate);
		}
		...
		void ListOnTapped(object sender, TappedRoutedEventArgs args)
		{
			...
			while (orig != null && orig != List)
			{
				...
				if (lv != null)
				{
					index = ((ITemplatedItemsView<Cell>)Element).TemplatedItems.GetGlobalIndexOfItem(lv.Content);
					break;
				}
	...
	// the listview has an item selected
			...
			((IListViewController)Element).InvokeNotifyRowTapped(index, cell: null);
