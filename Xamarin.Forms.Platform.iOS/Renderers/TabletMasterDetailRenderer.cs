	...
	public class TabletMasterDetailRenderer : UISplitViewController, IVisualElementRenderer, IEffectControlProvider
	{
		...
		public override void ViewDidLayoutSubviews()
		{
			...
			if (!masterBounds.IsEmpty)
				((IMasterDetailPageController)MasterDetailPage).MasterBounds = new Rectangle(0, 0, masterBounds.Width, masterBounds.Height);

			if (!detailsBounds.IsEmpty)
				((IMasterDetailPageController)MasterDetailPage).DetailBounds = new Rectangle(0, 0, detailsBounds.Width, detailsBounds.Height);
		}
		...
		public override void WillRotate(UIInterfaceOrientation toInterfaceOrientation, double duration)
		{
			...
			if (!((IMasterDetailPageController)MasterDetailPage).ShouldShowSplitMode && _masterVisible)
			{
				((IMasterDetailPageController)MasterDetailPage).CanChangeIsPresented = true;
			...
			((IMasterDetailPageController)MasterDetailPage).UpdateMasterBehavior();
		...
		{
			_masterVisible = true;
			if (((IMasterDetailPageController)MasterDetailPage).CanChangeIsPresented)
		...
		void ToggleMaster()
		{
			if (_masterVisible == MasterDetailPage.IsPresented || ((IMasterDetailPageController)MasterDetailPage).ShouldShowSplitMode)
				return;
